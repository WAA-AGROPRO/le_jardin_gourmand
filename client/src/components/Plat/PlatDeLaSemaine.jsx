...
function Plat() {
  ...
  useEffect(() => {
      async function fetchMenu() {
        const result = await axios.get('menu_de_la_semaine.json');
        setMenu(result.data);
      }
      fetchMenu();
    }, []);
  ...
  <div className="menu-plat">
    ...
    {menu.prix && (
      <section className="menu-plat__prices">
        <p className="menu-plat__price">{menu.prix.sur_place} </p>
        <p className="menu-plat__price">{menu.prix.a_emporter}</p>
        <p className="menu-plat__price">{menu.prix.livraison}</p>
      </section>  
    )}
